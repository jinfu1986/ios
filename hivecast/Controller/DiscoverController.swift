...
class DiscoverController: UIViewController, UITableViewDelegate, UITableViewDataSource, UISearchBarDelegate {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        Defaults[.streamUrl] = stream.streamUrl
        Defaults[.roomKey] = stream.key
        
        UserAPI.increaseVideoViewCount(videoId: stream.streamId, completion: { (result, errorMessage) in
            
        })

        self.performSegue(withIdentifier: "ToStreamPlaySegue", sender: self)
    }
